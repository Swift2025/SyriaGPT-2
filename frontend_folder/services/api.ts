...
const handleResponse = async (response: Response) => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = data.message || data.detail || data.error || 'An unknown error occurred.';
...
export const resetPassword = (token: string, new_password: string, confirm_password: string) => 
  apiFetch('/auth/reset-password', { method: 'POST', body: JSON.stringify({ token, new_password, confirm_password }) });

export const getCurrentUser = () => 
  apiFetch('/auth/me');

export const getGoogleAuthUrl = () => 
  apiFetch('/auth/oauth/google/authorize', { method: 'POST', body: JSON.stringify({}) });

export const getOAuthProviders = () => 
  apiFetch('/auth/oauth/providers');

export const oauthCallback = (provider: string, code: string, state?: string) => 
  apiFetch(`/auth/oauth/${provider}/callback`, { method: 'POST', body: JSON.stringify({ code, state }) });
...
export const askQuestion = (question: string, language: string = 'auto') => 
  apiFetch(`/intelligent-qa/ask?question=${encodeURIComponent(question)}&language=${language}`, { method: 'POST' });
...
export const getChatMessages = (chatId: string, page: number = 1, limit: number = 50) => 
  apiFetch(`/chat/${chatId}/messages?page=${page}&limit=${limit}`);

export const deleteMessage = (chatId: string, messageId: string) => 
  apiFetch(`/chat/${chatId}/messages/${messageId}`, { method: 'DELETE' });

export const clearChatHistory = (chatIds: string[] = []) => 
  apiFetch('/chat/bulk-action', { method: 'POST', body: JSON.stringify({ chat_ids: chatIds, action: 'delete' }) });
