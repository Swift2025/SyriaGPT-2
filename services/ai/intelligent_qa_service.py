...
class IntelligentQAService:
    ...
    async def process_question(
        ...
        **kwargs
        ...
        try:
            ...
            identity_response = identity_service.get_identity_response(question, language)
            ...
            syria_response = await self._search_syria_data(question)
            ...
            if search_results and search_results[0].get("similarity_score", 0) >= 0.85:
    ...
    async def _search_syria_data(self, question: str) -> Dict[str, Any]:
        """البحث في بيانات سوريا المحلية أولاً"""
        try:
            # 1. البحث الدلالي في Qdrant أولاً
            question_embedding = await embedding_service.generate_embedding(question)
            if question_embedding:
                search_results = await qdrant_service.search_similar_questions(
                    question_embedding,
                    limit=5,
                    score_threshold=self.medium_confidence_threshold
                )
                ...
                if search_results:
                    ...
                    if similarity_score >= self.medium_confidence_threshold:
                        answer_text = best_match.get("answer", "")
                        if answer_text and len(answer_text.strip()) > 10:
                            logger.info(f"✅ [SYRIA_DATA] Found answer in Qdrant (score: {similarity_score})")
                            return {
                                "status": "success",
                                "answer": answer_text,
                                "source": "syria_data_qdrant",
                                "confidence": similarity_score,
                                "category": best_match.get("metadata", {}).get("category", "syria")
                            }
            
            # 2. البحث المباشر في الملفات المحلية
            return await self._search_local_data_directly(question)
            
        except Exception as e:
            logger.error(f"Error in _search_syria_data: {e}")
            return {"status": "error", "error": str(e)}
    ...
    async def _search_local_data_directly(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # مسار البيانات
            data_path = Path(__file__).parent.parent.parent / "frontend_folder" / "public" / "data" / "syria_knowledge"
            knowledge_files = [
                "general.json",
                "cities.json", 
                "culture.json",
